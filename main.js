...
class ServiceNowAdapter extends EventEmitter {
  ...
  connect() {
    ...
    this.healthcheck((result,error)=>{
        if(result){
        console.log('This is from health check function success',result);
        }
        else{
            console.log('This is from health check function error',error);
        }
        });
  }
/**
...
if(error){
   ...
   } else if(error == 'Error present.') 
    {
       log.error(this.id,error);
       
    }
       else if(error == 'Bad response code.')
     {  
       log.error(this.id,error);
      ...
      */
      this.emitOnline();
      log.debug(this.id,'Successful working result as',result);
